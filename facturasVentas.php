		...
		<div class="container-fluid">
	    	<div class="collapse navbar-collapse" id="navbarResponsive">
	      		<ul class="navbar-nav ml-auto">
	        		...
	        		<li class="nav-item">
	          			<a class="nav-link fas fa-download" href="exportarFacturaVenta.php?IdProyecto=<?php echo $IdProyecto; ?>&ageCpra=<?php echo $ageCpra; ?>&mesCpra=<?php echo $mesCpra; ?>"> Descargar</a>
	        		</li>
        ...
        <?php
        	...
        	if(!$opciones){
	        	if(isset($IdProyecto)){ 
			        ...
			        if(isset($mesCpra))	{ $opciones .= " and month(fechaFactura) = '$mesCpra'"; }
			        if($ageCpra)	{ $opciones .= " and year(fechaFactura) = '$ageCpra'"; }
	        	}elseif(isset($mesCpra)){ 
					    if($mesCpra)	{ $opciones = " month(fechaFactura) = '$mesCpra'"; 	 }
					    if($ageCpra)	{ $opciones .= " and year(fechaFactura) = '$ageCpra'"; }
				}elseif(isset($ageCpra)){ $opciones = " year(fechaFactura) = '$ageCpra'"; }
        	}
        	$i = 0;
        	$sIva = 0;
  			$link=Conectarse();
        	$sql = "SELECT * FROM solfactura WHERE Factura = 'on' and nFactura > 0 and $opciones";
        	//echo $sql;
			$bd  = $link->query($sql);
			while($row=mysqli_fetch_array($bd)){
				...
				<tr class="<?php echo $class; ?>">
					...
					<td>
						<?php 
							if($row['fechaSolicitud'] != '0000-00-00'){
					...
					<td>
						<b>
						<?php 
							if($row['fechaFotocopia'] != '0000-00-00'){
	...
	<script>
		...
		app.controller('myCtrl', function($scope, $http) {
	  		$scope.cambiaropcion = function(ve){
	            $http.post('modeloVta.php',{nGasto:ve})
	            .then(function (res) {
