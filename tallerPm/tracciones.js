...
app.controller('ctrlEspectometro', function($scope, $http) {
    ...
    const $archivosSegTr = document.querySelector("#archivosSeguimientoT100");
    $scope.fileName = $archivosSegTr;
    $scope.enviarFormularioTr100 = function (evento) {

        
        let archivosSegTr = $archivosSegTr.files;
        if (archivosSegTr.length > 0) {
            let formdata = new FormData();
            
            angular.forEach(archivosSegTr, function (archivosSegTr) {
                formdata.append(archivosSegTr.name, archivosSegTr);
            });
            // alert($scope.Otam);
            ...
            var id = $scope.Otam;
            $http
                .post("guardar_archivosTraccionesTr100.php?Otam="+id, formdata, configuracion) 
                .then(function (respuesta) {
                    ...
                    // $scope.dataTraccionesTablaTr100();
                    window.location.href = 'pruebaQu.php?Otam='+$scope.Otam;
    ...
    $scope.cargarResultadosEnsayos = function(idItem){
        // alert('Tr100 '+$scope.Tr100);
        $scope.ficherosTracciones = true;
        $scope.idItem   = idItem;
        $scope.Otam     = idItem;
    }

    $scope.cargarResultadosEnsayosTr100 = function(idItem){
        $scope.ficherosTraccionesTr100 = true;
        $scope.idItem   = idItem;
        $scope.Otam     = idItem;
    }


    $scope.dataTraccionesTablaTr100 = function(){
        $http.post('dataTracciones.php',{
            Otam:       $scope.Otam,
            accion:     "dataTraccionesTablaTr100"
        })
        .then(function (response) {
            $scope.Prueba           = response.data.Prueba;
            $scope.idItem           = response.data.idItem;
            alert('Prueba..'+$scope.Prueba);
