...
app.controller('CtrlQuimicos', function($scope, $http) {
    ...
    $scope.iniciarValorCodInforme = function(CodInforme){
        $scope.CodInforme = CodInforme;
        // alert('Cod. '+$scope.CodInforme);
    }
    ...
    $scope.Actualizando = function(Estado){
        alert("Actualiza... "+$scope.Otam+' '+$scope.Programa+' '+$scope.Rep+' '+$scope.cC);  
        $http.post("dataRecord.php", { 
            Estado:         Estado,
            idItem:         $scope.Otam,
            Rep:            $scope.Rep,
