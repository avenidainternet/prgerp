...
<body>
	<?php
		...
		$nregistrosTr = 0;
		$bdEnc=$link->query("SELECT * FROM cotizaciones Where tpEnsayo > 2 and RAM > 0 and Estado = 'P' and usrResponzable in ('AVR', 'BAZ', 'SRS', 'JAG', 'PCP', 'DPG', 'GRC', 'ASH', 'CCE') Order By RAM Asc");
		while ($row=mysqli_fetch_array($bdEnc)){
			$nregistrosTr++;
		}
		$nPegas = 12;
		$nCol = ceil($nregistrosTr / $nPegas);
		// echo 'Total Reg/Col.: '.$nregistrosTr.'/'.$nCol;
	...
	$(document).ready(function(){
	  $('.toast').toast('show');
		...
		function mRAMs($il, $tl){
				...
				$bdEnc=$link->query("SELECT * FROM cotizaciones Where tpEnsayo > 2 and RAM > 0 and Estado = 'P' and usrResponzable in ('AVR', 'BAZ', 'SRS', 'JAG', 'PCP', 'DPG', 'GRC', 'ASH', 'CCE') Order By RAM Asc Limit $il, $tl");
...
function mRAMsAF($il, $tl){
		...
		$bdEnc=$link->query("SELECT * FROM Cotizaciones Where (tpEnsayo = 2 or tpEnsayo = 1) and RAM > 0 and Estado = 'P'  and usrResponzable in ('AVR', 'BAZ', 'SRS', 'JAG', 'PCP', 'DPG', 'GRC', 'ASH', 'CCE') Order By RAM Asc");
		if ($row=mysqli_fetch_array($bdEnc)){
			do{
