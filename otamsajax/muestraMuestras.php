		...
		*/
		...
		if($reg){
			...
			for($e=1; $e <= $cEnsayos; $e++ ){
				...
				if($idEnsayo == 'Qu'){
					if($tpMuestra == 'Ac'){
						$sqlor = "SELECT * FROM tablaorientacion Where idItem = '$idOtam'";
						$bdor=$link->query($sqlor);
						if($rsor=mysqli_fetch_array($bdor)){
						}else{
							$link->query("insert into tablaorientacion(idItem) values (	'$idItem' )");
						}
					}
					if($tpMuestra == 'Al'){
						$sqlor = "SELECT * FROM tablaorientacion Where idItem = '$idOtam'";
						...
						if($rsor=mysqli_fetch_array($bdor)){
							...
							$link->query("insert into tablaorientacion(idItem) values (	'$idOtam' )");
						}
					}
				}
							...
							<div class="card" style="margin: 10px;">
								...
								<div class="card-body" style="padding:10px;">
									...
									<div class="row">
										...
										<?php if($rowe['idEnsayo']=='Tr'){?>
											<div class="col-2" ng-show = 'equipoTr'>
												<div class="form-grup">
													<label for="equipo">Equipo:</label>

													<select name="equipo" class="form-control bg-secondary text-white">
															<?php
																$SQLeq = "SELECT * FROM equipos Where codigo like '%Tr%'";
																$bdeq=$link->query($SQLeq);
																while($rseq=mysqli_fetch_array($bdeq)){
																	if($rseq['codigo'] == $rowte['equipo']){?>
																		<option selected value="<?php echo $rseq['codigo']; ?>"><?php echo $rseq['codigo']; ?>
																		</option>
																		<?php
																	}
																	?>
																	<option value="<?php echo $rseq['codigo']; ?>"><?php echo $rseq['codigo']; ?>
																	</option>
																	<?php
																}
															?>
