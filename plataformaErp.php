			...
			<td width="200px" rowspan="5" valign="top">
				<?php include_once('menulateral.php');
					if($_SESSION['usr']=='GRC'){
						...
						$SQLm = "SELECT * FROM menugrupos Where nMenu = '8'";
						$bdm=$link->query($SQLm);
						if($rowm=mysqli_fetch_array($bdm)){?>
							<div id="MenuCuerpoTitulo" class="degradado">
								<?php echo $rowm['nomMenu']; ?>
							</div>
							<?php
						}
		
						$bdAC=$link->query("Select * From modulos Where nModulo = '34'");
						if($rs=mysqli_fetch_array($bdAC)){?>
							<span class="<?php echo $rs['iconoMod']; ?>"></span>
							<a href="<?php echo $rs['dirProg']; ?>" title="<?php echo $rs['Modulo']; ?>"> <?php echo $rs['Modulo']; ?></a>
							<?php
						}
						$link->close();
					}
